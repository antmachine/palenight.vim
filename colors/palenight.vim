...
call s:h("CursorColumn", { "fg": s:black, "bg": s:red }) " the screen column that the cursor is in when 'cursorcolumn' is set
call s:h("CursorLine", { "fg": s:black, "bg": s:red }) " the screen line that the cursor is in when 'cursorline' is set
...
call s:h("TabLineFill", {s:comment_grey}) " tab pages line, where there are no labels
